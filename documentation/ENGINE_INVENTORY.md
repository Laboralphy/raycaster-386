...
| **1. Rendering** — `@laboralphy/raycaster386` | Turning world state into pixels: map rendering, textures, lighting, sprites, level loading. | a screen | Time. Input. I/O. |
| **2. Simulation** — `@laboralphy/raycaster386/simulation` | Advancing world state by a tick: doors, movement, collision, triggers, actors. | a clock | Importing Rendering. Owning a loop. Touching the DOM. |
